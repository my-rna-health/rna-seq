workflow cleanup {
    ...
    scatter (sample in condition) {
        ...
        if(sample != condition[0]) {

            call get_sample {input: condition = sample[0], sample = sample }

            call report as initial_report {
                input:
                    sampleName = basename(get_sample.out[0][1]),
                    file = get_sample.out[0][1]
            }
...
task get_sample {
  ...
  command {
    --filetype fastq --keep_sra false ${sample}
  }

  runtime {
    docker: "quay.io/comp-bio-aging/geoparse@sha256:bdf03cda576e24985060a795bede5b50eca89c9053a0fe179b8f9bc282e4db00"
  }

  output {
    Array[Array[String]] out = read_tsv("./results/file_list.tsv")
  }

...
workflow chip {
    ...
    call build_index {input: fasta = reference, name = basename(reference, ".fa"), threads = aligner_threads}
    ...
    call bowtie1 as align_treatment {
        input:
...
task build_index {
    ...
    command {
       bowtie-build --large-index --threads ~{threads} ~{fasta} ~{name}
    }

    runtime {
       docker: "biocontainers/bowtie@sha256:c7cd608a10d63ed2d6ad664ec6a8b0d6e689018dd6aa9af294040483f8df28cf"
       maxRetries: 2
     }
...
task bowtie1 {
       ...
       String index = basename(fasta, ".fa")

       command {
           bowtie-build --large-index --threads ~{threads} ~{fasta} ~{index_name}
           bowtie1 -p ~{threads} ~{index} \
           -r ~{sep='\t' reads} \
           -S  ~{name}.sam
       }
